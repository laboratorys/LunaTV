...
function HomeClient() {
  ...
  const [bangumiCalendarData, setBangumiCalendarData] = useState<
    BangumiCalendarData[]
  >([]);
  const [hotShortDrama, setHotShortDrama] = useState<ShortDramaItem[]>([]);
  ...
  const GetHotShortDramaData = async () => {
    const resp = await fetch('/api/short-drama', {
      method: 'GET',
      ...
      <div className='px-2 sm:px-10 py-4 sm:py-8 overflow-visible'>
        ...
        <div className='mb-8 flex justify-center'>
          <CapsuleSwitch
            options={[
              { label: '首页', value: 'home' },
              { label: '播放记录', value: 'history' },
              { label: '收藏夹', value: 'favorites' },
            ]}
            active={activeTab}
            onChange={(value) =>
              setActiveTab(value as 'home' | 'favorites' | 'history')
            }
          />
        </div>
        ...
        <div className='max-w-[95%] mx-auto'>
          ...
          {activeTab === 'home' && (
            ...
            <>
              ...
              {/* 短剧 */}
              {mounted && cfg?.SHOW_SHORT_DRAMA !== false && (
                <section className='mb-8'>
                  <div className='mb-4 flex items-center justify-between'>
                    <h2 className='flex items-center text-xl font-bold text-gray-800 dark:text-gray-200'>
